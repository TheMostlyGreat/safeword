  ...
  for (const skillDirectory of skillDirectoryectories) {
    describe(`skill: ${skillDirectory}`, () => {
      ...
      it("should have valid name format (lowercase, a-z0-9-)", () => {
        ...
        if (name) {
          expect(name, `name "${name}" contains invalid characters`).toMatch(
            SKILL_NAME_PATTERN,
          );
        }
      });
      ...
      it("should have name under 64 characters", () => {
        ...
        if (name) {
          expect(
            name.length,
            `name is ${name.length} chars, max is 64`,
          ).toBeLessThanOrEqual(SKILL_NAME_MAX_LENGTH);
        }
      });
      ...
      it('should use third person in description (not "I" or "you")', () => {
        ...
        if (desc) {
          ...
          expect(desc, 'description should not start with "I "').not.toMatch(
            /^I\s/,
          );
          expect(desc, 'description should not contain " I "').not.toMatch(
            /\sI\s/,
          );
          // Allow "you" in trigger examples like "'help you'"
          // but flag direct second person like "You can" or "helps you"
          expect(desc, 'description should avoid "You can" style').not.toMatch(
            /\bYou can\b/i,
          );
        }
      });
  ...
  for (const commandFile of commandFiles) {
    describe(`command: ${commandFile}`, () => {
      ...
      it("should have lowercase filename", () => {
        expect(commandFile, "Command filename should be lowercase").toBe(
          commandFile.toLowerCase(),
        );
      });
      ...
      it("should have markdown body (not just frontmatter)", () => {
        if (parsed) {
          expect(
            parsed.body,
            "Command needs content after frontmatter",
          ).not.toBe("");
          expect(
            parsed.body?.length,
            "Body should have content",
      ...
      it("should not use $0 (invalid argument)", () => {
        ...
        expect(content, "Use $1, $2, etc. or $ARGUMENTS, not $0").not.toContain(
          "$0",
        );
      });
      ...
      it("should have valid command name format", () => {
        ...
        expect(
          commandName,
          `Command name "${commandName}" should be lowercase`,
        ).toBe(commandName.toLowerCase());
        expect(
          commandName,
      ...
      it("should have valid argument-hint format if present", () => {
        ...
        if (argumentHint !== undefined) {
          expect(typeof argumentHint, "argument-hint should be a string").toBe(
            "string",
          );
          // argument-hint should describe expected arguments
          expect(
            argumentHint.length,
            "argument-hint should have content",
          ).toBeGreaterThan(0);
        }
      });
  ...
  for (const ruleFile of ruleFiles) {
    describe(`rule: ${ruleFile}`, () => {
      ...
      it("should have lowercase filename with hyphens", () => {
        expect(ruleFile, "Filename should be lowercase").toBe(
          ruleFile.toLowerCase(),
        );
        expect(ruleName, "Name should only contain a-z, 0-9, -").toMatch(
          /^[a-z0-9-]+$/,
        );
      });
      ...
      it("should have description as string if present", () => {
        ...
        if (description !== undefined) {
          expect(typeof description, "description should be string").toBe(
            "string",
          );
...
describe("Skills-Cursor Parity", () => {
  it("each safeword skill should have corresponding cursor rule", () => {
    ...
    const ruleFiles = new Set(
      getCursorRuleFiles().map((f) => nodePath.basename(f, ".mdc")),
    );
...
describe("Validation Logic Tests", () => {
  describe("Skill Validation Logic", () => {
    ...
    it("detects invalid characters in skill name", () => {
      ...
      for (const name of invalidNames) {
        expect(name, `"${name}" should be invalid`).not.toMatch(
          SKILL_NAME_PATTERN,
        );
      }
    });
    ...
    it("detects reserved words in name", () => {
      ...
      for (const name of reservedNames) {
        ...
        expect(hasReserved, `"${name}" should contain reserved word`).toBe(
          true,
        );
      }
    });
    ...
    it("detects invalid allowed-tools format", () => {
      ...
      for (const format of invalidFormats) {
        expect(format, `"${format}" should be invalid`).not.toMatch(
    ...
    it("validates correct allowed-tools formats", () => {
      ...
      for (const format of validFormats) {
        expect(format, `"${format}" should be valid`).toMatch(
          ALLOWED_TOOLS_PATTERN,
        );
      }
    });
  ...
  describe("Gerund Naming Convention", () => {
    ...
    it("rejects non-gerund names", () => {
      ...
      for (const name of invalidNames) {
        expect(isGerundName(name), `"${name}" should not be gerund`).toBe(
          false,
        );
      }
    });
