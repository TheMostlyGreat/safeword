  ...
  describe('detectArchitecture()', () => {
    ...
    describe('standard project structure', () => {
      ...
      it('detects root-level directories without src/', () => {
        ...
        expect(result.elements.find(e => e.layer === 'utils')?.pattern).toBe('utils/**');
        expect(result.elements.find(e => e.layer === 'components')?.pattern).toBe('components/**');
      });
      ...
      it('detects alternative directory names', () => {
        ...
        expect(result.elements.find(e => e.layer === 'utils')?.location).toBe('src/helpers');
        expect(result.elements.find(e => e.layer === 'components')?.location).toBe('src/ui');
        expect(result.elements.find(e => e.layer === 'services')?.location).toBe('src/api');
      });
      ...
      it('prefers src/ over root level for same layer', () => {
        ...
        const utilitiesElements = result.elements.filter(e => e.layer === 'utils');
  ...
  describe('generateBoundariesConfig()', () => {
    ...
    describe('single layer', () => {
      it('generates config with single element', () => {
        const arch = {
          elements: [{ layer: 'utils' as const, pattern: 'src/utils/**', location: 'src/utils' }],
          isMonorepo: false,
        };
    ...
    describe('multiple layers with rules', () => {
      it('generates hierarchy rules for components importing utils', () => {
        const arch = {
          elements: [
            { layer: 'utils' as const, pattern: 'src/utils/**', location: 'src/utils' },
            {
      ...
      it('does not generate rule for types layer (no imports allowed)', () => {
        const arch = {
          elements: [
            { layer: 'types' as const, pattern: 'src/types/**', location: 'src/types' },
            { layer: 'utils' as const, pattern: 'src/utils/**', location: 'src/utils' },
          ],
          isMonorepo: false,
        };
      ...
      it('generates full hierarchy for complete architecture', () => {
        const arch = {
          elements: [
            { layer: 'types' as const, pattern: 'src/types/**', location: 'src/types' },
            { layer: 'utils' as const, pattern: 'src/utils/**', location: 'src/utils' },
            { layer: 'services' as const, pattern: 'src/services/**', location: 'src/services' },
            {
              layer: 'components' as const,
              pattern: 'src/components/**',
              location: 'src/components',
            },
            { layer: 'features' as const, pattern: 'src/features/**', location: 'src/features' },
          ],
          isMonorepo: false,
        };
