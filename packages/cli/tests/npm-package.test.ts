...
describe("NPM Package Structure", () => {
  it("should have package.json with correct files array", () => {
    const packageJson = JSON.parse(
      readFileSync(nodePath.join(cliRoot, "package.json"), "utf8"),
    );
  ...
  it("should resolve templates from dist context", () => {
    ...
    const templatesFromDistribution = nodePath.join(
    ...
    expect(
      existsSync(nodePath.join(templatesFromDistribution, "SAFEWORD.md")),
    ).toBe(true);
  });
});
