...
function createWorkspacePackage(dir: string, name: string, scripts: Record<string, string> = {}): void {
  ...
  writeFileSync(nodePath.join(pkgDir, 'package.json'), JSON.stringify(pkg, undefined, 2));
}
...
describe('Setup - Workspace Format Scripts', () => {
  ...
  it('should add format script to workspace packages without one', async () => {
    // Create monorepo structure
    createMonorepoPackageJson(temporaryDirectory, ['packages/app', 'packages/lib']);
    createWorkspacePackage(temporaryDirectory, 'packages/app', { build: 'tsc' });
    createWorkspacePackage(temporaryDirectory, 'packages/lib', { build: 'tsc' });
    ...
    const libraryPackage = JSON.parse(readTestFile(temporaryDirectory, 'packages/lib/package.json'));
  ...
  it('should not overwrite existing format script', async () => {
    ...
    createWorkspacePackage(temporaryDirectory, 'packages/app', { format: 'biome format .' });
    initGitRepo(temporaryDirectory);
  ...
  it('should handle glob workspace patterns', async () => {
    ...
    const corePackage = JSON.parse(readTestFile(temporaryDirectory, 'packages/core/package.json'));
    const utilitiesPackage = JSON.parse(readTestFile(temporaryDirectory, 'packages/utils/package.json'));

    expect(corePackage.scripts.format).toBe('prettier --write .');
    expect(utilitiesPackage.scripts.format).toBe('prettier --write .');
  }, TIMEOUT_SETUP);

  it('should skip if root has existing formatter config', async () => {
    createMonorepoPackageJson(temporaryDirectory, ['packages/app']);
  ...
  it('should handle Yarn workspace object format', async () => {
    ...
    writeFileSync(nodePath.join(temporaryDirectory, 'package.json'), JSON.stringify(pkg, undefined, 2));
    createWorkspacePackage(temporaryDirectory, 'packages/app', {});
    initGitRepo(temporaryDirectory);
  ...
  it('should gracefully skip non-existent workspace directories', async () => {
    ...
    const existsPackage = JSON.parse(readTestFile(temporaryDirectory, 'packages/exists/package.json'));
