...
describe("Test Suite: Setup - Cursor IDE Support", () => {
  ...
  describe("Cursor Rules", () => {
    it("should create .cursor/rules/safeword-core.mdc", async () => {
      ...
      expect(
        fileExists(temporaryDirectory, ".cursor/rules/safeword-core.mdc"),
      ).toBe(true);
  ...
  describe("Cursor Commands", () => {
    it("should create .cursor/commands/ directory with command files", async () => {
      ...
      expect(fileExists(temporaryDirectory, ".cursor/commands/lint.md")).toBe(
        true,
      );
      expect(
        fileExists(temporaryDirectory, ".cursor/commands/quality-review.md"),
      ).toBe(true);
      expect(fileExists(temporaryDirectory, ".cursor/commands/drift.md")).toBe(
        true,
      );
    });
    ...
    it("should have same content as Claude commands", async () => {
      ...
      const claudeLint = readTestFile(
        temporaryDirectory,
        ".claude/commands/lint.md",
      );
      const cursorLint = readTestFile(
        temporaryDirectory,
  ...
  describe("Cursor Hooks", () => {
    it("should create .cursor/hooks.json with version and hooks", async () => {
      ...
      const hooksConfig = JSON.parse(
        readTestFile(temporaryDirectory, ".cursor/hooks.json"),
      );
    ...
    it("should create Cursor hook scripts in .safeword/hooks/cursor/", async () => {
      ...
      expect(
        fileExists(temporaryDirectory, ".safeword/hooks/cursor/stop.ts"),
      ).toBe(true);
    });
    ...
    it("should reference correct hook script paths in hooks.json", async () => {
      ...
      expect(hooksConfig.hooks.stop[0].command).toBe(
        "bun ./.safeword/hooks/cursor/stop.ts",
      );
    });
  });
  ...
  describe("Cursor MCP Configuration", () => {
    it("should create .cursor/mcp.json with same servers as .mcp.json", async () => {
      ...
      const cursorMcp = JSON.parse(
        readTestFile(temporaryDirectory, ".cursor/mcp.json"),
      );
      const claudeMcp = JSON.parse(
        readTestFile(temporaryDirectory, ".mcp.json"),
      );

      expect(cursorMcp.mcpServers.context7).toEqual(
        claudeMcp.mcpServers.context7,
      );
      expect(cursorMcp.mcpServers.playwright).toEqual(
        claudeMcp.mcpServers.playwright,
      );
    });
  });
  ...
  describe("Reset removes Cursor files", () => {
    ...
    it("should remove .safeword/hooks/cursor/ on reset", async () => {
      ...
      await runCli(["setup"], { cwd: temporaryDirectory });
      expect(fileExists(temporaryDirectory, ".safeword/hooks/cursor")).toBe(
        true,
      );
