...
describe("Reset Command - Reconcile Integration", () => {
  ...
  function createConfiguredProject() {
    ...
    mkdirSync(
      nodePath.join(temporaryDirectory, ".safeword-project/tickets/completed"),
      {
        recursive: true,
      },
    );
    writeFileSync(
      nodePath.join(temporaryDirectory, ".safeword/version"),
      "0.6.3",
    );
    writeFileSync(
      ...
      JSON.stringify(
        {
          hooks: {
            SessionStart: [
              {
                hooks: [
                  { type: "command", command: ".safeword/hooks/test.sh" },
                ],
              },
