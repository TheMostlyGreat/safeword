...
describe("TanStack Query rules at error severity", () => {
  it("@tanstack/query/exhaustive-deps is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(tanstackQueryConfig, "@tanstack/query/exhaustive-deps"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("@tanstack/query/no-void-query-fn is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(tanstackQueryConfig, "@tanstack/query/no-void-query-fn"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("@tanstack/query/no-unstable-deps is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(tanstackQueryConfig, "@tanstack/query/no-unstable-deps"),
      ),
    ).toBe(ERROR);
  });
...
describe("TanStack Query has all 7 rules configured", () => {
  it("has exactly 7 @tanstack/query rules", () => {
    ...
    const queryRules = Object.keys(allRules).filter((rule) =>
      rule.startsWith("@tanstack/query/"),
    );
