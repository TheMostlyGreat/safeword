...
describe("Vitest critical rules at error", () => {
  it("vitest/expect-expect is at error", () => {
    expect(
      getSeverityNumber(getRuleConfig(vitestConfig, "vitest/expect-expect")),
    ).toBe(ERROR);
  });

  it("vitest/no-focused-tests is at error", () => {
    expect(
      getSeverityNumber(getRuleConfig(vitestConfig, "vitest/no-focused-tests")),
    ).toBe(ERROR);
  });
  ...
  it("vitest/no-identical-title is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(vitestConfig, "vitest/no-identical-title"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("vitest/valid-expect is at error", () => {
    expect(
      getSeverityNumber(getRuleConfig(vitestConfig, "vitest/valid-expect")),
    ).toBe(ERROR);
  });
});
  ...
  it("targets test files", () => {
    const hasTestFilePattern = playwrightConfig.some(
      (config) =>
        ...
        config.files.some(
          (f: string) =>
            f.includes(".test.") || f.includes(".spec.") || f.includes(".e2e."),
        ),
    );
    expect(hasTestFilePattern).toBe(true);
  });
...
describe("Playwright critical rules at error", () => {
  it("playwright/expect-expect is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(playwrightConfig, "playwright/expect-expect"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("playwright/no-focused-test is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(playwrightConfig, "playwright/no-focused-test"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("playwright/valid-expect is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(playwrightConfig, "playwright/valid-expect"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("playwright/no-wait-for-timeout is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(playwrightConfig, "playwright/no-wait-for-timeout"),
      ),
    ).toBe(ERROR);
  });
  ...
  it("playwright/no-page-pause is at error", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(playwrightConfig, "playwright/no-page-pause"),
      ),
    ).toBe(ERROR);
  });
});
...
describe("Playwright no-skipped-test exception", () => {
  it("playwright/no-skipped-test stays at warn (TDD exception)", () => {
    expect(
      getSeverityNumber(
        getRuleConfig(playwrightConfig, "playwright/no-skipped-test"),
      ),
    ).toBe(WARN);
  });
});
