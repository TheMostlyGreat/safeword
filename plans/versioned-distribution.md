# Versioned Distribution Plan for .agents

**Goal:** Make .agents distributable to external users with proper versioning, zero global dependencies, and minimal bloat.
...
│   ├── auto-quality-review.sh       # Generated from heredoc in setup-quality-review.sh
│   └── run-quality-review.sh        # Generated from heredoc in setup-quality-review.sh
├── commands/
│   ├── lint.md                      # Generated from heredoc in setup-linting.sh
│   └── quality-review.md            # Generated from heredoc in setup-quality-review.sh
└── settings.json                    # Merged by both scripts
```

**Key principles:**
- No global folders (no ~/.agents for external users)
...
### Task 1: Create `setup-quality-review.sh`
**Status:** Not started
...
1. `.claude/hooks/auto-quality-review.sh`
   ...
   # To update: curl https://raw.../v1.0.0/coding/setup-quality-review.sh | bash
...
**Critical:**
- Use `$CLAUDE_PROJECT_DIR/.claude/hooks/...` not `/Users/alex/.agents/...`
- All code must be inline via heredocs (no external file references)
- Script must work without any ~/.agents folder existing
...
**Status:** Not started
**Problem:** Both `setup-linting.sh` and `setup-quality-review.sh` write to `.claude/settings.json`
...
- Implement same pattern in setup-quality-review.sh
...
# setup-quality-review.sh adds Stop:
...
vim coding/setup-linting.sh          # Change VERSION="v1.1.0"
vim coding/setup-quality-review.sh   # Change VERSION="v1.1.0"
...
4. Delete old reference to `/Users/alex/.agents/...`
...
**Status:** Not started
**Modify:** Both `setup-linting.sh` and `setup-quality-review.sh`
...
2. **Update linting-hook-setup.md**
...
**Distribution:**
- [ ] External user can run single curl command to set up linting (no ~/.agents folder needed)
- [ ] External user can run single curl command to set up quality review (no ~/.agents folder needed)
...
**Generated Files:**
- [ ] All hooks are project-local (no `~/.agents` references, no global folders)
...
- [ ] setup-quality-review.sh creates `.claude/commands/quality-review.md` automatically
...
1. Create `setup-quality-review.sh`
...
3. Verify no references to `/Users/alex/.agents/`
...
3. **Chat repo uses Biome via ultracite:** Should setup-quality-review.sh work with both?
...
- **Day 1:** Create `setup-quality-review.sh` (Task 1)
...
**Critical path:**
1. Task 1: Create setup-quality-review.sh (enables migration)
