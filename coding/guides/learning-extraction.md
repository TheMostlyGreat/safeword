...
**LLM Instruction Design:** Learnings are documentation that LLMs read and follow. Apply best practices from `@~/.claude/llm-instruction-design.md` when writing learning files (concrete examples, explicit definitions, MECE principles).
...
**Project learnings** (`./agents/learnings/[concept].md`):
...
**Historical archives** (`./agents/learnings/archive/[bug-fix].md`):
...
ls ./agents/learnings/*keyword*.md
```
...
│  ├─ YES → Continue
│  └─ NO → agents/learnings/archive/[bug-fix].md (optional)
│
...
### Forward-Looking Learning (agents/learnings/)
...
See `agents/learnings/archive/[investigation].md` for full debugging narrative.
```

### Debugging Narrative (agents/learnings/archive/)
...
Project-specific gotchas in `agents/learnings/`:

- **Persistent UI Placement** - Controls in LayoutBar (always visible), not EditorTabBar (conditional) → `agents/learnings/persistent-ui.md`
- **Electron Renderer Context** - Renderer = browser, not Node.js; use `split(/[/\\]/)` for paths → `agents/learnings/electron-contexts.md`
...
### ✅ Project Learning (agents/learnings/)
...
### ❌ Archive (agents/learnings/archive/)
...
agents/learnings/electron-gotchas.md

# BETTER (3 focused files)
agents/learnings/electron-renderer-context.md      (80 lines)
agents/learnings/electron-ipc-patterns.md         (60 lines)
agents/learnings/electron-path-validation.md      (50 lines)
```
