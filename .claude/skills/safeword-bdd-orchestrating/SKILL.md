...
| **TDD Loop** | >5 unit tests for single E2E          | Break E2E into smaller steps |

### Entry Checkpoint: Examples + Reasoning

**Before suggesting a split, reason through the request:**

```
STEP 1: Count distinct user stories
- List each "As a [user] I want [thing]" you can identify
- 1 story → feature
...
STEP 2: Check against examples
- Does this match epic or feature patterns below?

STEP 3: Assess depth
- How many sequential steps in the longest flow?
- Deeper flows (6+ steps, state machines) → likely epic
```

**Epic examples (suggest split):**

| Request                     | Why Epic                                                         |
| --------------------------- | ---------------------------------------------------------------- |
| "Add user authentication"   | Multiple flows (login, logout, session, register), state machine |
| "Build analytics dashboard" | Multiple views, data sources, user roles                         |
| "Add e-commerce checkout"   | Cart, payment, inventory, shipping - many concerns               |

**Feature examples (proceed as-is):**

| Request                | Why Feature                            |
| ---------------------- | -------------------------------------- |
| "Add password reset"   | Single flow, clear boundaries          |
| "Add dark mode toggle" | One state change, affects styling only |
| "Add export to CSV"    | Single action, well-defined output     |

**Two-story example (suggest split into 2 features):**

Request: "Add password reset and email verification"

- Story 1: "As a user, I want to reset my password"
- Story 2: "As a user, I want to verify my email"
- These are **parallel** (independent flows) → suggest 2 feature tickets

**Two-story example (keep as 1 feature):**

Request: "Add login with remember me"

- Story 1: "As a user, I want to log in"
- Story 2: "As a user, I want to stay logged in"
- These are **coupled** (shared session state) → 1 feature with 2 journeys

### Split Protocol

When user accepts a split:

**If no ticket exists yet:**

1. Create epic ticket with `children:` array
2. Create child tickets with `parent:` link
3. Commit: "chore: split [name] into N features"
4. Ask: "Which feature should we start with?"

**If ticket already exists (promote to epic):**

1. Update existing ticket: `type: feature` → `type: epic`
2. Add `children:` array with new child ticket IDs
3. Create child tickets with `parent:` linking to promoted ticket
4. Add work log entry: "Promoted to epic, split into N features"
5. Commit: "chore: promote [id] to epic, split into N features"
6. Ask: "Which feature should we start with?"
...
| Phase 6  | `implement`         | Stay in implementation                  |
| TDD Loop | `implement`         | Stay in TDD loop                        |

### Example: Entry Split (New Ticket)
...
### Example: Entry Split (Existing Ticket Promoted)

```
Before:
015-account-security
├── type: feature
└── phase: intake

After:
015-account-security (promoted)
├── type: epic          ← changed
├── children: [016, 017] ← added
│
├── 016-password-reset
│   ├── parent: 015
│   └── phase: intake
...
### Phase 6 & TDD Loop Checkpoints

These are **lightweight splits** - no new tickets, just reorganize work:

**Phase 6 (>10 tests per slice):**

- Add `## Implementation Slices` section to test-definitions
- Each slice gets its own subsection with status marker
- Example:

  ```markdown
  ## Implementation Slices

  ### Slice 1: Auth Layer [in_progress]

  - [x] Scenario 1.1
  - [ ] Scenario 1.2

  ### Slice 2: Validation Layer [pending]
...
- Break E2E into intermediate checkpoints
- Each intermediate step gets its own commit
- Example: Instead of one E2E "user completes checkout", break into:
  1. E2E: User adds item to cart
  2. E2E: User enters shipping info
  3. E2E: User completes payment

### Split Suggestion UX

When threshold is breached, present split suggestion in two stages:

**Stage 1: Summary (always show)**

```
"This looks like [N] distinct [features/journeys/slices].

Threshold breached: [which threshold]
Recommendation: Split into [N] [features/slices]

[Accept] [See details] [Proceed anyway]"
```

**Stage 2: Details (on request)**

If user selects "See details", show proposed structure:

```
"Proposed split:

Feature 1: Password Reset
- Scenarios: 1-5 (request, email, validation, success, errors)
- Components: auth-service, email-service, reset-form

Feature 2: Email Verification
- Scenarios: 6-10 (signup, verify, resend, expiry, errors)
- Components: auth-service, email-service, verify-flow

Each becomes a separate ticket with its own BDD cycle."
```

### User Override

User can always decline a split:

**"Proceed anyway"** → Agent continues with original scope:

- Log in work log: "Split suggested (N [features/slices]) but user declined"
- Continue at current phase
- Do NOT re-suggest at same checkpoint in same session

**Re-suggestion at later checkpoint:**

- If user declined at Entry but Phase 3 triggers (>15 scenarios)
- Agent CAN suggest again with note: "Previously declined split at Entry. Phase 3 now has [N] scenarios, which also triggers split suggestion."
...
- **Split** → check thresholds at Entry, Phase 3, Phase 5, Phase 6, TDD Loop; user decides
