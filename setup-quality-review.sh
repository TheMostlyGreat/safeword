...
VERSION_HEADER="# Generated by setup-quality-review.sh $VERSION
...
# ============================================================================
# Step 2: Copy AGENTS.md and guides for standalone operation
# ============================================================================
echo "[2/5] Setting up project documentation..."

# Find the script's directory to locate source files
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Copy global patterns to .safeword/ (standalone, no global folder dependency)
mkdir -p .safeword
if [ -f "$SCRIPT_DIR/AGENTS.md" ]; then
  cp "$SCRIPT_DIR/AGENTS.md" .safeword/SAFEWORD.md
  echo "  ✓ Copied global patterns to .safeword/SAFEWORD.md"
else
  echo "  ⚠ Warning: $SCRIPT_DIR/AGENTS.md not found (skipping)"
fi

# Always copy/update guides (reference documentation)
if [ -d "$SCRIPT_DIR/guides" ]; then
  cp -r "$SCRIPT_DIR/guides" .safeword/
  echo "  ✓ Copied guides to .safeword/guides/"
else
  echo "  ⚠ Warning: $SCRIPT_DIR/guides not found (skipping)"
fi

# Add SAFEWORD.md reference to existing files or create new AGENTS.md
if [ -f CLAUDE.md ]; then
  # CLAUDE.md exists - check if it already has the trigger
  if grep -q "@./.safeword/SAFEWORD.md" CLAUDE.md; then
    echo "  ✓ CLAUDE.md already has SAFEWORD.md reference (skipped)"
  else
    # Prepend trigger at the top
    temp_file=$(mktemp)
    cat > "$temp_file" << 'EOF'
...
EOF
    ...
    echo "  ✓ Added SAFEWORD.md reference to existing CLAUDE.md"
  fi
elif [ -f AGENTS.md ]; then
  # AGENTS.md exists - check if it already has the trigger
...
EOF
  ...
  # Neither exists - create AGENTS.md with trigger
  cat > AGENTS.md << 'EOF'
...
# ============================================================================
echo "[4/5] Generating quality review hooks..."
...
# Only runs for projects with AGENTS.md, CLAUDE.md, or SAFEWORD.md (searches upward)
...
# Check for project-level AGENTS.md, CLAUDE.md, or SAFEWORD.md
# Search upward from current directory (supports monorepos)
current_dir="$PWD"
echo "Searching for AGENTS.md, CLAUDE.md, or SAFEWORD.md..." >> "$DEBUG_LOG"
while true; do
  if [ -f "$current_dir/AGENTS.md" ] || [ -f "$current_dir/CLAUDE.md" ] || [ -f "$current_dir/SAFEWORD.md" ]; then
  ...
  if [ "$current_dir" = "/" ]; then
    echo "No AGENTS.md, CLAUDE.md, or SAFEWORD.md found, exiting" >> "$DEBUG_LOG"
    exit 0
  fi
...
echo "  • Global patterns copied to .safeword/SAFEWORD.md"
echo "  • Reference documentation copied to .safeword/guides/"
echo "  • Project context file: AGENTS.md or CLAUDE.md (references @./.safeword/SAFEWORD.md)"
echo "  • All files are project-local and standalone"
echo ""
echo "⚠ IMPORTANT:"
echo "  • COMMIT .safeword/ and .claude/ to your repo for team consistency"
echo "  • Claude Code will read @./.safeword/SAFEWORD.md via AGENTS.md or CLAUDE.md reference"
echo "  • No external dependencies - project is fully portable"
echo ""
echo "Hook behavior:"
echo "  • Only runs in projects with AGENTS.md, CLAUDE.md, or SAFEWORD.md"
